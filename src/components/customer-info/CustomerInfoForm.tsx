...
export function CustomerInfoForm() {
  ...
  const getFieldsForPage = (page: number): (keyof FormData)[] => {
    switch (page) {
      ...
      case 4:
        return ['purchaseGiftCards', 'selectedGiftCards', 'acceptedTerms'];
      default:
        return [];
    }
  };
  ...
  // Page 2: Date of Birth, Parish, Gender
  const renderPage2 = () => (
    <Card className="bg-slate-800/50 border-slate-600 backdrop-blur-sm hover:border-emerald-400/50 transition-all duration-300">
      <CardHeader className="border-b border-slate-600">
        <CardTitle className="text-2xl font-bold text-emerald-400 flex items-center gap-3">
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                <FormControl>
                  <SelectTrigger className="bg-slate-700 border-slate-600 text-white focus:border-emerald-400 focus:ring-emerald-400/20 transition-all duration-200">
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-purple-900 text-white">
      <div className="max-w-4xl mx-auto p-6 space-y-6">
        ...
        <div className="w-full bg-slate-800 rounded-full h-2 mb-8">
          <div className="bg-gradient-to-r from-emerald-400 to-cyan-400 h-2 rounded-full transition-all duration-500" style={{ width: `${getProgressPercentage()}%` }} />
        </div>
          ...
          >
            ...
            <div className="flex justify-between pt-6">
              <Button
                ...
                className="px-8 py-3 text-lg font-semibold border-slate-600 text-slate-300 hover:bg-slate-700 hover:text-white transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Previous
              </Button>
